...
function Notifications::OnTankSpawned::TankSpawn( tank, params )
{
	...
	if ( SessionState.AllowTankSplit )
	{
		if ( SessionState.TankSplitLevel == 1 )
		{
			...
			tank.SetHealth(3000);
			tank.InputColor(255, 85, 0); //orange
			//tank.InputColor(143, 51, 247); //purple
			splitAmount = 2;
		}
		else if ( SessionState.TankSplitLevel == 2 )
		{
			...
			tank.SetHealth(1500);
			tank.InputColor(189, 74, 255); //new purple
			//tank.InputColor(18, 255, 34); //green
			splitAmount = 4;
		}
		else if ( SessionState.TankSplitLevel == 3 )
		{
			...
			tank.SetHealth(500);
			tank.InputColor(0, 255, 255); //blue
			//tank.InputColor(255, 255, 0); //yellow
			splitAmount = 4;
		}
	...
	else
	{
		...
		else
		{
			...
			tank.InputColor(255, 69, 169); //pink
			tank.SetHealth(6000);
		}
	}
	Timers.AddTimer(30.0, false, KillInfected, { infected = tank, attacker = null });
}
...
function Notifications::OnTankKilled::TankDeath( tank, attacker, params )
{
	...
	if ( splitAmount == 2 )
	{
		Utils.SpawnZombie( Z_TANK, tank.GetLocation() + Vector( 10, 0, 0 ) );
		Utils.SpawnZombie( Z_TANK, tank.GetLocation() + Vector( -10, 0, 0 ) );
	}
	else if ( splitAmount == 3 )
	{
		Utils.SpawnZombie( Z_TANK, tank.GetLocation() + Vector( 10, 0, 0 ) );
		Utils.SpawnZombie( Z_TANK, tank.GetLocation() + Vector( -10, 0, 0 ) );
		Utils.SpawnZombie( Z_TANK, tank.GetLocation() + Vector( 20, 0, 0 ) );
	}
