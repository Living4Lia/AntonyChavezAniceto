...
function EasyLogic::OnTakeDamage::AllowDamage( damageTable )
{
	try
	{
		...
		if ( attacker.GetType() >= 1 && attacker.GetType() <= 6 )
		{
			if ( attacker.GetType() == Z_SPITTER && damageTable.DamageType != DMG_CLUB )
				return true;
			else
			{
		...
		if ( attacker.IsSurvivor() && victim.GetType() >= 1 && victim.GetType() <= 8 )
			return false;
	}
	catch( error )
	{
...
{
	foreach ( jimmy in Objects.OfModel("models/infected/common_male_jimmy.mdl") )
		jimmy.Input( "Kill" );
}
